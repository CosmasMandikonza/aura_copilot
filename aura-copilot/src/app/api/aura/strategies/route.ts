// src/app/api/aura/strategies/route.ts
import { NextRequest, NextResponse } from 'next/server'
import { getStrategies } from '@/lib/aura'
...
export async function GET(req: NextRequest) {
  ...
  const data = await getStrategies(address)
  await prisma.cacheEntry.create({ data: { address, kind: 'strategies', payload: data } })
